 ...
 * @returns {string} 报告文本。
 */
export function renderSessionReport(session, analysis, health = analysis?.health) {
  ...
  // 适配器可能给不出这些字段 —— generic 路径就没有 project / modifiedAt。
  // 报告只跳过那一行，不崩：IR 只保证 id / cwd / runtime 一定有。
  const projectName = session.cwd || (session.project ?? '').replace(/^--|-{2}$/g, '');
  if (projectName) lines.push(`  工作目录 ${projectName}`);
  if (session.modifiedAt) lines.push(`  时间     ${session.modifiedAt.toLocaleString('zh-CN')}`);
  if (session.runtime) lines.push(`  运行时   ${session.runtime}`);
  if (sessions) lines.push(`  日志     ${sessions.decodedFrames} 帧 / ${num(sessions.bytes)} 字节解压`);
  lines.push('');

  // ── 可信度自检 ────────────────────────────────────────
  // 放在最前面而不是末尾：一个数字偏低的报告比一个崩溃的工具危险得多 ——
  // 前者看起来完全正常，使用者会照着它做决策。
  if (health && health.warnings.length > 0) {
    lines.push('  ⚠️  可信度警告');
    for (const warning of health.warnings) {
      lines.push(`    ${warning.level === 'high' ? '❗' : '·'} ${warning.text}`);
    }
    lines.push('');
  }
...
export function renderWorkflow(session, workspace, habits, advice) {
  ...
  // 判定用「每条平均值」而不是占比：占比的分母会被助手输出撑大，会话越长数字
  // 越小，无法反映"是否在重述背景"。占比降级为参考项，并且明确标注它的局限。
  const perMessage = Math.round(habits.userTokensPerMessage ?? 0);
  const restatementFlag = perMessage > 250 ? '  ⚠️ 偏高' : '  ✓';
  lines.push(`    背景重述成本   ${pad(`${perMessage}/条`, 8, 'right')}${restatementFlag}`
    + `   用户消息平均体积（高 = 每次都在重新交代背景）`);
  lines.push(`    用户消息占比   ${pad(`${((habits.restatementRatio ?? 0) * 100).toFixed(1)}%`, 8, 'right')}`
    + `   参考项：占比随会话变长而下降，不可用于判断趋势`);
 ...
 * @param prompt - {@link import('./prompt.js').inspectPrompt} 的结果。
 * @param advice - {@link import('./prompt.js').promptAdvice} 的结果。
 * @returns 多行文本。
 */
export function renderPrompt(session, prompt, advice) {
  const lines = [];
  lines.push('');
  lines.push('  ctxgc · 常驻成本审计');
  lines.push('─'.repeat(78));
  if (session) {
    lines.push(`  会话     ${session.id}`);
    lines.push(`  工作目录 ${session.cwd}`);
  }
  lines.push(`  轮数     ${num(prompt.turns)}（这些内容每一轮都在上下文里，所以按轮数计费）`);
  lines.push('');

  lines.push('  每轮固定开销');
  lines.push(`    ${pad('部分', 14)}${pad('项数', 6, 'right')}${pad('字符', 10, 'right')}`
    + `${pad('≈token/轮', 12, 'right')}${pad('本会话累计', 14, 'right')}`);
  for (const part of prompt.parts) {
    lines.push(`    ${pad(part.label, 14)}${pad(num(part.count), 6, 'right')}`
      + `${pad(num(part.chars), 10, 'right')}${pad(num(part.tokens), 12, 'right')}`
      + `${pad(formatTokens(part.tokens * prompt.turns), 14, 'right')}`);
  }
  lines.push(`    ${'─'.repeat(64)}`);
  lines.push(`    ${pad('合计', 14)}${pad('', 6)}${pad('', 10)}`
    + `${pad(num(prompt.perTurn), 12, 'right')}${pad(formatTokens(prompt.total), 14, 'right')}`);
  lines.push('');

  lines.push('    说明：token 是估算（CJK 约 0.7/字，其余约 4 字符/token），不是 DSH 上报的真实用量。');
  lines.push(`          会话日志只落了「系统提示」这一块（${num(prompt.visibleFromLog)} token/轮）；`);
  lines.push('          工具定义在 request/header 事件里，工作区约定在文件系统里 ——');
  lines.push('          少取这两处，每轮固定开销会被低估到实际的三分之一。');
  lines.push('');

  if (prompt.tools.items.length > 0) {
    lines.push('  工具定义排行（用不到的工具插件是纯支出）');
    prompt.tools.items.slice(0, 10).forEach((tool, i) => {
      const pct = ((tool.chars / prompt.tools.chars) * 100).toFixed(1);
      lines.push(`    ${pad(String(i + 1), 4, 'right')} ${pad(truncateDisplay(tool.name, 20), 22)}`
        + `${pad(num(tool.chars), 8, 'right')} 字符${pad(`${pct}%`, 7, 'right')}`
        + `${pad(`${num(tool.tokens)}/轮`, 12, 'right')}`);
    });
    lines.push('');
  }

  if (prompt.agents.items.length > 0) {
    lines.push('  工作区约定明细（完全由你控制）');
    for (const item of prompt.agents.items) {
      lines.push(`    ${pad(truncateDisplay(item.path, 46), 48)}${pad(`${num(item.tokens)}/轮`, 12, 'right')}`);
    }
    lines.push('');
  }
  ...
  if (advice && advice.length > 0) {
    ...
    for (const a of advice) {
      lines.push(`    ${a.level === 'high' ? '⚠️  ' : '·  '}${a.text}`);
    }
    lines.push('');
  }
