...
export function inspectWorkspace(root) {
  ...
  for (const [key, spec] of Object.entries(WORKSPACE_SLOTS)) {
    ...
    if (found && found.isDir) {
      try {
        // 递归统计：产出归档的定义就是「一次工作一个文件夹」，只数顶层会把
        // 子目录里的产出全部漏掉 —— 实测 sessions/ 下有 3 个文件却只报 1 个、
        // 体积报 0.0 KB，体检结论因此失真（还显示为 ✅ 已达标）。
        for (const entry of readdirSync(found.path, { recursive: true })) {
...
export function analyzeHabits(analysis) {
  ...
  const userTokens = sum(userItems.map((i) => i.tokens));
  ...
  return {
    ...
    assistantTokens,
    /** 用户消息条数（配合平均值一起读）。 */
    userMessages: userItems.length,
    /**
     * 背景重述的**稳定**指标：每条用户消息的平均体积。
     *
     * 判断趋势要用这个，不要用下面的占比 —— 占比的分母会被每轮累积的助手输出
     * 撑大。实测同一个会话里从 9.0% 掉到 2.0%，而用户消息的绝对长度毫无变化。
     * 平均值不受会话长度影响，跨时段、跨会话都可比。
     */
    userTokensPerMessage: userItems.length > 0 ? userTokens / userItems.length : 0,
    /**
     * 用户消息占上下文的比例。**仅作参考**：它回答"上下文被谁占据"，
     * 不回答"用户是不是在反复交代背景"。
     */
...
export function recommendations(workspace, habits) {
  ...
  if (habits.userTokensPerMessage > 250) {
    out.push({
      level: 'high',
      text: `背景重述：用户消息平均 ${Math.round(habits.userTokensPerMessage)} token/条（共 ${habits.userMessages} 条）—— 偏长，说明每次都在重新交代背景，应写进 memory/ 让入口自动加载`,
    });
  }
